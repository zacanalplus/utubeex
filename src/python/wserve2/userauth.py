    ...
    def pure_search(self, keyQry : dict) -> register_response:
        ...
        if(o_found):
            for obj in o_found:
                ...
                self.intent = obj["intent"]
                #debug
                print("Debugging print [field : appuser.intent] = [value :" + self.intent + "]")
                print("Debugging print [func : self.has_wintent()] = [value :" + str(self.has_wintent()) + "]")
                rres.int_ok = self.has_wintent()
                return rres
        return None
